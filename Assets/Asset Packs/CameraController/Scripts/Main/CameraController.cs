...
namespace Bizniz
{
    public partial class CameraController : MonoBehaviour
    {
        ...
        public Rewired.Player Rewiredplayer
        {
            get
            {
                if (m_Rewiredplayer == null)
                    m_Rewiredplayer = ReInput.players.GetPlayer(RewiredPlayerId);
...
#else
                if (Rewiredplayer.GetButton(Camera_Profile.SpeedBoost))
                    return (Camera_Profile.Move_Speed * 100f) * Camera_Profile.Move_SpeedMulti * Camera_Profile.BoostMove_SpeedMulti;
                else return (Camera_Profile.Move_Speed * 100f) * Camera_Profile.Move_SpeedMulti;
#endif
            }
...
#else
            if (Rewiredplayer.GetButtonDown(Camera_Profile.Drag))
            {
                CamOrigin = GetOrigin();
            }
            if (Rewiredplayer.GetButton(Camera_Profile.Drag))
            {
                Difference = GetDifference();
                Camera_ToUse.transform.position = CamOrigin - Difference;
            }
...
#else
            if (Rewiredplayer.GetButton(Camera_Profile.Rotate_Left))
            {
                Camera_ToUse.transform.Rotate(Vector3.up * Rotate_SpeedValue() * Time.deltaTime, Space.World);
            }
            else if (Rewiredplayer.GetButton(Camera_Profile.Rotate_Right))
            {
                Camera_ToUse.transform.Rotate(-Vector3.up * Rotate_SpeedValue() * Time.deltaTime, Space.World);
            }
#endif
        ...
        protected virtual void Update_Zooming()
        {
...
#else
            //Wait for Inputs and Zoom the Camera Up/Down!
            if (Input.GetAxis(Camera_Profile.MouseScrollWheelInputName) > 0f || Rewiredplayer.GetButton(Camera_Profile.ZoomIn))
            {
                if (Camera_ToUse.orthographic)
                    Camera_ToUse.orthographicSize -= Zoom_SpeedValue() * Time.deltaTime;
                else
            ...
            else if (Input.GetAxis(Camera_Profile.MouseScrollWheelInputName) < 0f || Rewiredplayer.GetButton(Camera_Profile.ZoomOut))
            {
                if (Camera_ToUse.orthographic)
                    Camera_ToUse.orthographicSize += Zoom_SpeedValue() * Time.deltaTime;
                else
